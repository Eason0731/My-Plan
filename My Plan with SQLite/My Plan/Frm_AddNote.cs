...
namespace My_Plan
{
    public partial class Frm_AddNote : Form
    {
        ...
        {
            string connectString = "data source = MyData.db" ;
            SQLiteConnection myconn = new SQLiteConnection(connectString);
            ...
            else
            {
                ...
                {
                    Content = txtContent.BodyInnerHTML.Replace("'", "''"); //将单引号被替换为两个单引号的结果，存到临时变量中，该临时变量作为插入内容。每次遇到有单引号的字符，则需要通过该方法去替换
                }

                else
                {
                    Content = txtContent.BodyInnerHTML; //若字符中不包含单引号，则还是按照文本框输入的字符进入插入
                }
                ...
                SQLiteCommand myCmd = new SQLiteCommand(insStr, myconn);
                myCmd.ExecuteNonQuery();
                myconn.Close();
                MessageBox.Show("添加笔记成功!");

                txtTitle.Text = "";
                txtContent.BodyInnerHTML = ""; //txtContent.BodyHtml = "";
